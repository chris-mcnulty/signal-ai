 ...
 * These pages share an identical featured-lead + list structure and differ
 * only in copy, the API category filter, and their test-id prefix.
 */
export interface CategoryPageProps {
  /** Category value passed to the articles API. */
  category: string;
  /** Small uppercase eyebrow above the title. */
  eyebrow: string;
  /** Page title. */
  title: string;
  /** Descriptive standfirst under the title. */
  dek: string;
  /** Heading for the secondary list section (e.g. "Recent News"). */
  recentLabel: string;
  /** Prefix for card test-ids, e.g. "news" → `link-news-<slug>`. */
  testidPrefix: string;
  /** Heading shown when there is no content. */
  emptyHeading: string;
  /** Body shown when there is no content. */
  emptyBody: string;
}
      ...
      <main className="max-w-[1200px] mx-auto p-6 md:p-12 min-h-[60vh]">
        ...
        {isLoading ? (
          ...
          <>
            {featured && (
              <Link
                ...
                data-testid={`link-${testidPrefix}-featured-${featured.slug}`}
              >
                <article>
            ...
            {rest.length > 0 && (
              <section className="animate-fade-in-up">
                <div className="border-t-4 border-news pt-2 mb-8">
                  <h3 className="font-mono text-xs font-bold uppercase tracking-widest">{recentLabel}</h3>
                </div>
                <div className="space-y-0">
                  {rest.map((article, index) => (
                    <Link
                      ...
                      data-testid={`link-${testidPrefix}-${article.slug}`}
                    >
          ...
          <div className="border border-dashed border-news py-32 text-center animate-fade-in-up">
            <p className="font-mono text-sm uppercase tracking-widest text-news-secondary mb-2">{emptyHeading}</p>
            <p className="font-serif text-news-secondary italic text-lg">{emptyBody}</p>
          </div>
        )}
      </main>
